...
angle_buoy_bf = angle_buoy( find( year(1:end-1) <= 2007 & (month(1:end-1)>=8 & month(1:end-1)<=9) ) );
angle_buoy_aft = angle_buoy(find(year(1:end-1)>2007 & (month(1:end-1)>=8 & month(1:end-1)<=9) ));

%curve
figure(1)
[N,edges] = histcounts(angle_buoy_bf, 'Normalization', 'probability');
edges = edges(2:end) - (edges(2)-edges(1))/2;
plot(edges, N, 'LineWidth',2);
hold on;
grid on;
[N,edges] = histcounts(angle_buoy_aft, 'Normalization', 'probability');
edges = edges(2:end) - (edges(2)-edges(1))/2;
plot(edges, N, 'LineWidth',2);
hold off;

xlabel('angle','FontSize',17)
ylabel('probability density','FontSize',17)
legend('before 2007', 'after 2007', 'FontSize',16)
title('Normal Probability Density', 'FontSize',20)
