...
%month - leap year_o.ver



%month = leap yearx.ver

month_day = [31 28 31 30 31 30 31 31 30 31 30 31];

%1월
for i = 1:42
    month_lyx_1 = find(cum_doy == 1+365*(i-1) : 31+365*(i-1) );
end

month_lyx = NaN*zeros(42,11);

%2월~12월
for i = 0:41 
    for j = 2:12
%             month_lyx(i+1,j-1) = find(cum_doy == (1+sum(month_day(1:j-1)))+365*i : (sum(month_day(1:j)))+365*i );
         month_lyx(i+1,j-1) = ((1+sum(month_day(1:j-1)))+365*i : (sum(month_day(1:j)))+365*i );
...
ncfile_n_u='uvel_1979-2020_NSIDC.nc';

un=ncread(ncfile_n_u,'u');
unlat = ncread(ncfile_n_u, 'lat');
unlon = ncread(ncfile_n_u, 'lon');

ncfile_n_v='vvel_1979-2020_NSIDC.nc';

vn=ncread(ncfile_n_v,'v');
vnlat = ncread(ncfile_n_v, 'lat');
vnlon = ncread(ncfile_n_v, 'lon');

%% importing ice drift speed data - 수치모델(cesm) %%

ncfile_c_u='uvel_1979-2020_CESM.nc';

uc=ncread(ncfile_c_u,'uvel');
uclat = ncread(ncfile_c_u, 'lat');
uclon = ncread(ncfile_c_u, 'lon');

ncfile_c_v='vvel_1979-2020_CESM.nc';

vc=ncread(ncfile_c_v,'vvel');
vclat = ncread(ncfile_c_v, 'lat');
vclon = ncread(ncfile_c_v, 'lon');
